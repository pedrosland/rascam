    ...
    D: de::Deserializer<'de>,
{
    ...
    impl<'de> Visitor<'de> for HexVisitor {
        ...
        {
            if &value[0..2] != "0x" {
                return Err(E::custom(format!("expected hex value to start with \"0x\": {}", value)))
            }
...
pub (crate) fn probe(path: PathBuf) -> Result<ProbeInfo, String> {
    let args = ["-v", "error", "-count_frames", "-show_streams", "-print_format", "json"]
        .iter()
        .map(OsString::from)
    ...
    let stderr = String::from_utf8(output.stderr.clone())
        .unwrap_or_else(|binary| format!("{:?}", binary));
    ...
    if !output.status.success() {
        ...
        return Err(format!("failed to execute ffprobe: exit status: {}, stderr: {}, stdout: {}", code, stderr, stdout));
    }
...
pub (crate) fn probe_stream(path: PathBuf) -> Result<ProbeStream, String> {
    ...
    if info.streams.len() != 1 {
        return Err(format!("expected exactly 1 stream, got {} streams", info.streams.len()));
    }
...
pub (crate) fn assert_framerate(actual: f32, expected: u32) {
    ...
    if diff > allowed_variance {
        assert!(false, "actual framerate is {}, expected +/- 10% of {}", actual, expected);
    }
}
